...
void mini_log_flush(void)
{
    ...
    while ((n = fifo_uni_read_block(&s_ring, chunk, (uint16_t)sizeof(chunk))) > 0)
    {
...
void mini_log_default_output(const char *str, ...)
{
    ...
    uint16_t used = fifo_uni_get_count(&s_ring);
    if ((uint16_t)(MINI_LOG_RING_SIZE - used) < len)
        return;

    fifo_uni_write_block(&s_ring, fmt, len);
...
void mini_log_flash_output(const char *str, ...)
{
    ...
    uint16_t used = fifo_uni_get_count(&s_flash_ring);
    if ((uint16_t)(MINI_LOG_FLASH_RING_SIZE - used) < len)
        return;

    fifo_uni_write_block(&s_flash_ring, fmt, len);
...
int mini_log_flash_flush(void)
{
    ...
    while ((n = fifo_uni_read_block(&s_flash_ring, chunk, (uint16_t)sizeof(chunk))) > 0)
    {
